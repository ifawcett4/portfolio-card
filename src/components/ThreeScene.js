...
export default function SceneCanvas() {
  ...
  const handleMouseMove = (event) => {
    ...
    const xRot = THREE.MathUtils.clamp(nx * Math.PI * 0.5, -Math.PI / 4, Math.PI / 4);
    const yRot = THREE.MathUtils.clamp(ny * Math.PI * 0.25 + Math.PI / 6, -Math.PI / 2, Math.PI / 2);
  ...
  function ModelWrapper() {
    ...
    useFrame(() => {
      ...
      ref.current.rotation.x = THREE.MathUtils.lerp(ref.current.rotation.x, tx, 0.08);
      ref.current.rotation.y = THREE.MathUtils.lerp(ref.current.rotation.y, ty, 0.08);
    });
    return (
      <group ref={ref}>
        <Abstract />
